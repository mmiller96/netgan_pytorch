...
def score_matrix_from_random_walks(random_walks, N, weights_walks, symmetric=True):
    """
    ...
    weights_walks = np.vstack(weights_walks)[:, 1:, :].flatten()
    ...
    for i, (x, y) in enumerate(bigrams):
        weight_matrix[x, y] += weights_walks[i]
    ...
    mat = sp.coo_matrix((np.ones(bigrams.shape[0]), (bigrams[:, 0], bigrams[:, 1])),
                        shape=[N, N])
    weight_matrix = np.divide(weight_matrix, mat.toarray(), where=mat.toarray()!=0)
    return mat, weight_matrix

#@jit(nopython=True)
def random_walk(edges, graph_weighted, node_ixs, rwlen, p=1, q=1, n_walks=1):
    N=len(node_ixs)
    ...
    for w in range(n_walks):
        ...
        for it in range(rwlen-1):
            ...
            walk.append(nxt)
            prev_nbs = set(nbs)
    rw_complete = np.array(walk).reshape([-1, rwlen])
    weights = np.zeros((rw_complete.shape[0], rw_complete.shape[1], 1))
    weights[:, 1:, :] = np.expand_dims(long_rw_to_single_rw(rw_complete, graph_weighted), axis=2)
    return rw_complete, weights

class RandomWalker:
    """
    Helper class to generate random walks on the input adjacency matrix.
    """
    def __init__(self, adj, adj_weighted, rw_len, p=1, q=1, batch_size=128):
        self.adj = adj
        ...
        self.adj_weighted = adj_weighted

    def walk(self):
        return random_walk(self.edges, self.adj_weighted, self.node_ixs, self.rw_len, self.p, self.q, self.batch_size)
        #while True:
        #    yield random_walk(self.edges, self.adj_weighted, self.node_ixs, self.rw_len, self.p, self.q, self.batch_size).reshape([-1, self.rw_len])
...
def get_graph_weighted(path_folder, tag):
    ...
    data_branch['l_scaled'] = scaler.fit_transform(data_branch['l'].values.reshape(-1, 1))

    edges_weighted = data_branch[['fbus', 'tbus', 'l_scaled']].values
    ...
    G_weighted = nx.to_scipy_sparse_matrix(G_weighted)
    G_weighted = G_weighted.todense()
    return G, G_weighted, scaler

def calc_lines_mse(rw_list, graph_weighted_real, graph_weighted_fake):
    weights_diff = np.zeros(len(rw_list))
    for i, rw_single in enumerate(rw_list):
        weights_diff[i] = graph_weighted_real[rw_single[0], rw_single[1]] - graph_weighted_fake[rw_single[0], rw_single[1]]
    loss = (weights_diff**2).sum()/len(rw_list)
    return loss

def get_weights_from_rw_list(rw_list, graph_weighted):
    weights = np.zeros(len(rw_list))
    for i, (x, y) in enumerate(rw_list):
        weights[i] = graph_weighted[x, y]
    return weights

def long_rw_to_single_rw(rw_complete, graph_weighted):
    rw_list = []
    for i in range(rw_complete.shape[0]):
        for j in range(rw_complete.shape[1]-1):
            rw_list.append(np.sort(rw_complete[i, j:j+2]))
    rw_list = np.vstack(rw_list)
    weight_list = get_weights_from_rw_list(rw_list, graph_weighted)
    weight_matrix = weight_list.reshape(-1, rw_complete.shape[1] - 1)
    return weight_matrix

def create_train_graph(path_folder, tag, trainer_best):
    path_branch = path_folder + r'\branch.csv'
    path_bus = path_folder + r'\bus.csv'
    ...
    data_branch.loc[data_branch['l']==0.0, 'l'] = 0.1

    data_branch['l_inverse'] = 1/data_branch['l']

    edges_weighted = data_branch[['fbus', 'tbus', 'l_inverse']].values
    nodes = np.arange(data_bus.shape[0])
    ...
    train_graph = trainer_best.train_graph.toarray()*nx.convert_matrix.to_numpy_array(G_weighted)
    train_graph = nx.convert_matrix.from_numpy_array(train_graph)

    nx.write_gexf(train_graph, "originals.gexf")


def clean_weighted_graph(graph, graph_weighted, scaler):
    if type(graph) is np.ndarray or type(graph) is np.matrix:
        graph = sp.csr_matrix(graph)
    if type(graph_weighted) is np.ndarray or type(graph_weighted) is np.matrix:
        graph_weighted = sp.csr_matrix(graph_weighted)
    lcc = largest_connected_components(graph)
    graph = graph[lcc, :][:, lcc]
    graph_weighted = graph_weighted[lcc, :][:, lcc]
    graph_weighted = scaler.inverse_transform(graph_weighted.toarray()) * graph.toarray()
    graph_weighted[(graph_weighted <= 0.0)] = 0.0
    lcc = largest_connected_components(graph_weighted)
    graph_weighted = graph_weighted[lcc, :][:, lcc]
    graph_weighted_inverse = graph_weighted.copy()
    graph_weighted_inverse[graph_weighted_inverse > 0.0] = 1 / graph_weighted_inverse[graph_weighted_inverse > 0.0]

    graph = nx.convert_matrix.from_scipy_sparse_matrix(graph)
    graph_weighted = nx.convert_matrix.from_numpy_array(graph_weighted)
    graph_weighted_inverse = nx.convert_matrix.from_numpy_array(graph_weighted_inverse)
    return graph, graph_weighted, graph_weighted_inverse